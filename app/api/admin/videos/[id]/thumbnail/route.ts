...
/** 管理画面から動画サムネイル画像をアップロード（JPEG/PNG/WebP/GIF、最大5MB） */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!videoId) return NextResponse.json({ error: "id が必要です" }, { status: 400 });
  ...
  const storagePath = `video-thumbnails/${videoId}/thumbnail.${ext}`;
